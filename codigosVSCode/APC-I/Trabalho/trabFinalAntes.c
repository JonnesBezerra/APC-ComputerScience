...
void sem_isolamento() {
	...
	for(seg=0;seg<1801;seg++){
		...
		infec=0;
		cont_doentes=0;
		//Verificando a posiзгo dos saudбveis
		for(i=0;i<100;i++){
			for(j=0;j<100;j++){
				if(cidade[i][j]==1){
					pos_LinhaSaud[a]=i;
		...
		//Movimentaзгo dos saudaveis
		for(c=0;c<a;c++){
			//Garantindo que o saudбvel nгo esteja cercado por outras pessoas
			...
			//Segundo caso: Primeira Linha
			else if(pos_LinhaSaud[c]==0){
			...
			//Primeira coluna entre as linhas 0 e 9
			else if(pos_ColunaSaud[c]==0){
...
void parcial(){
	...
	for (seg = 0; seg < 1801; seg++){
		...
		// Define quais saudaveis vão se movimentar
        int sorteio;
        int mov = 0; // mov contém o total de indivíduos saudáveis que se movimentam
        int pos_mov[5001]; // pos_mov contém a posição de quem vai se movimentar nos vetores de posição dos saudáveis
		for (int z = 0; z < a; z++){
            sorteio = 1 + rand() % 100;
			if (sorteio <= 50){ //basta mudar o valor aqui para fazer 20%, 50% ou 100% de movimentação
                pos_mov[mov] = z;
                mov++;
			}
        }

        //Moviemtnacao dos saudaveis
		for (int z = 0; z < mov; z++){
            // c contém o índice no vetor de saudáveis do saudável que irá se movimentar
            c = pos_mov[z];
		...
		//MovimentaÁ„o dos infectados
		for (c = 0; c < infec; c++){
			//Garantindo que o infectado n„o esteja cercado por outras pessoas
			//Primeiro caso: 2 Nao estando nas bordas
			if ((pos_LinhaInfec[c] > 0) && (pos_LinhaInfec[c] < 99) && (pos_ColunaInfec[c] > 0) && (pos_ColunaInfec[c] < 99)){
				if ((cidade[pos_LinhaInfec[c]+1][pos_ColunaInfec[c]] == 0) || (cidade[pos_LinhaInfec[c]-1][pos_ColunaInfec[c]] == 0) || (cidade[pos_LinhaInfec[c]][pos_ColunaInfec[c]+1] == 0) || (cidade[pos_LinhaInfec[c]][pos_ColunaInfec[c]-1] == 0)){
					cidade[pos_LinhaInfec[c]][pos_ColunaInfec[c]] = 0;
...
void maximo(){
	...
	for (seg = 0; seg < 1801; seg++){
		...
		for (int z = 0; z < a; z++){
            sorteio = 1 + rand() % 100;
			if (sorteio <= 20){ //basta mudar o valor aqui para fazer 20%, 50% ou 100% de movimentação
                pos_mov[mov] = z;
                mov++;
			}
        }
